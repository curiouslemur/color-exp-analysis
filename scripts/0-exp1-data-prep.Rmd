...
data prep needs to be done only once. 
Outputs are written to the data folder since they will be used in subsequent analysis
...
- ``mean_rating`` (prev. "weight") is the average rating across all participants for each {color, concept} pair
...
```{r write data}
# write_csv(usdem, "data/exp1/csv/us-dem.csv")
# write_csv(usdf, "data/exp1/csv/us-df.csv")
# 
# write_csv(mgdem, "data/exp1/csv/mg-dem.csv")
# write_csv(mgdf, "data/exp1/csv/mg-df.csv")
```

## Computing weights and SE
```{r}
mgdf_w <- mgdf %>% group_by(concept, color) %>% 
  summarize(mean_rating = round(mean(rating), digits = 4),
...
# write_csv(usdf_w, "data/exp1/csv/us-df-summary.csv")
# write_csv(mgdf_w, "data/exp1/csv/mg-df-summary.csv")
```
