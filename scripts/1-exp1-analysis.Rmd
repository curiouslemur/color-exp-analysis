...
Estimating the value of alpha in the fit for standard deviation.
```{r US data}
...
a_us <- coef(getFitSd(usdf_agg)) # alpha a_us = 1.384426 which is ~ 1.4
# Plotting the data points, and fitted curve
plot(usdf_agg$xbar, usdf_agg$sd_sample,
  main = "Fit for model s = a * xbar * (1 - xbar)",
...
# Plotting the data points, and fitted curve
plot(mgdf_agg$xbar, mgdf_agg$sd_sample,
  main = "MDG data: Fit for the Function s = a * xbar * (1 - xbar)",
  xlab = "Mean (xbar)", ylab = "Standard Deviation (s)", pch = 16
)
