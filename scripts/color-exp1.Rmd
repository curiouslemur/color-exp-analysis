...
p <- en_debug %>% 
  ...
  # filter(concept %in% conceptNames) %>%
  ggplot(aes(code, rating, color = hex)) + 
  # coord_flip() +
  geom_jitter(alpha = 0.5, width = 0.5, size = 0.75)+
  stat_summary(fun.data = "mean_cl_boot", size = 0.55) + 
  scale_color_identity() +
  # facet_wrap(.~concept, ncol = 2) + theme_studyBackground()
  facet_grid(factor(concept, levels=conceptNames)~expCountry); p
  # scale_y_continuous(expand = c(0, 0), limits = c(0, 1.05))
```
```{r}
ggsave(file="./fig/ci-weights.png", plot = p, width=16, height=20)
```

Comparing ratings between the two languages
```{r}
q <- en_debug %>% 
  ...
  facet_wrap(~factor(concept, levels=conceptNames)) + 
...
Comparing the differences between the means
```{r}
