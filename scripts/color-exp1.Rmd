...
- *Bars ordered by increasing rating*
```{r using tidytext - bars ordered by weight ratings}
plot <- df_weights %>%
  # filter(concept %in% c("carrot","justice", "comfort", 'peace')) %>%
  filter(expCountry == 'mdg') %>%
ggplot(aes(x = reorder_within(code, weight, concept), y = weight, fill = code)) + # reoder color codes BY weight WITHIN each concept
    geom_bar(stat = 'identity', color = 'black', width = 0.95, size = 0.25) + 
    scale_x_reordered() +
    scale_fill_manual(values = df_weights$hex) + 
    scale_y_continuous(expand = c(0, 0), limits = c(0, 1)) +
    # facet_wrap(expCountry~concept, scales = "free_x", ncol = 1, strip.position = 'right') +
    facet_wrap( ~concept, scales = 'free_x', ncol = 4, strip.position = 'top') + 
    # facet_grid( concept~expCountry, scales = 'free_x') +
...
```{ eval=FALSE}
ggsave(file="./fig/20231214-weights-err-bars.png", plot = plot, width=20, height=16)
```

### Sample exploration

CI for the mean with jitters
```{r echo=FALSE}
pd <- position_dodge(0.0)
en_debug <- full_join(en_debug, bcp37, by = 'code')
en_debug$rating <- en_debug$answer/100

en_debug$code = fct_rev(factor(en_debug$code, levels = color_codes))

p <- en_debug %>% 
  # filter(expCountry == 'usa') %>%
  # filter(concept %in% conceptList) %>%
  ggplot(aes(code, rating, color = hex)) + 
  # coord_flip() +
...
```
```{eval=FALSE}
ggsave(file="./fig/ci-weights.png", plot = p, width=18, height=20)
```

Comparing ratings between the two languages in overlapping 95% CI
```{r}
q <- en_debug %>% 
  # filter(concept %in% c("banana", "peace")) %>%
  ggplot(aes(code, rating, color = expCountry)) +
  coord_flip() +
  stat_summary(data = filter(en_debug, expCountry == "mdg"), fun.data = "mean_cl_boot",  size = 0.45, alpha = 1, position = position_nudge(x = 0.15, y = 0), geom = "pointrange")  +
  ...
  facet_wrap(~factor(concept, levels=conceptList)) + 
...
t.res <- full_join(t.res, bcp37, by = 'code')
...
## Plotting
t.res$code = fct_rev(factor(t.res$code, levels = color_codes))
t.res <- t.res %>% mutate(ciColor = if_else(p.value <= 0.05, hex, 'black'))


r <- t.res %>% 
  filter(p.value <= 0.05) %>%
  ggplot(aes(code, meanDiff, color = ciColor)) + 
  ...
  ggtitle("95% CI for the difference between the mean for MDG - USA"); r
```
```{r eval=FALSE}
ggsave(file="./fig/ci-mean-diff.png", plot = r, width=16, height=22)
ggsave(file="./fig/ci-mean-diff.svg", plot = r, width=10, height=14)
