...
"debug" is the dataset for trials
"debrief" is the dataset for demographics
...
en_mdg <- filter(en_debug, grepl("mdg", expCountry))
en_mdg$expCountry <- "mdg"
en_usa <- filter(en_debug, !grepl("mdg", expCountry))
en_usa$expCountry <- "usa"
...
```{r data prep}
df_weights <- df %>%
  group_by(expCountry, concept, color) %>%
  summarize(
...
t.res <- df_pivot %>%
  group_by(concept, color) %>%
  summarise(
    t = round(t.test(as.vector(unlist(mg)), as.vector(unlist(usa)))$statistic, digits = 4),
...
r <- t.res %>%
  ...
  labs(title = "95% CI for the difference between the mean for MDG - USA", y = "", x = "color code in BCP37 library")
r
```
...
df_weights %>% 
  filter(concept %in% c("angry", "banana"), expCountry == "mdg") %>%
  ggplot(aes(x = reorder(color, weight), y = weight, fill = color)) +
